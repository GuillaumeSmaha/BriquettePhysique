...
\chapter{Choix techniques}
    ...
    \section{Liste des outils}
		...
		pour une question de sauvegarde des données, nous utiliserons le site
		...
		Nous avons utilisé pour se faire le logiciel Doxygen\refMot{bib:outils_doxygen}.
...
\newpage
    ...
    \section{Calcul du score}
        L'objectif est de construire la flèche la plus éloigné possible du bord
        ...
        "C" (Calcul) pour voir un menus donnant le score s'afficher.
        Celui-ci est calculer à partir de la briquette stable la plus éloigné
        de la table. Si la briquette vient d'être placé et qu'elle bouge encore
        un peu, elle ne sera pas comptabilisé immédiatement. Cela permet
...
\chapter{Conclusion}
    ...
    collectif\refMot{bib:projet_rv} nous à permis de gagner en efficacité et de mener
    le projet à bien. En particulier nous avons implémenter correctement la
    physique du jeu. Nous avons profiter de cette aisance pour offrir de
    nombreuses améliorations, aidant la prise en mains du jeu (par exemple la
    possibilité d'arreter le moteur physique, permettant de placer plusieurs
    briquettes avant le relancer la physique).
    ...
    de tester les solutions ainsi obtenu. On pourrait également travaillé pour
